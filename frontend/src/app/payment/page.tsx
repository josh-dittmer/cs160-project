...
export default function PaymentPage() {
  ...
  const totalWeightOz = 
    cartItems?.reduce(
        (sum, item) => sum + (item.item.weight_oz ?? 0) * item.quantity,
        0
    ) ?? 0;
  // convert oz to lbs
  ...
  const shippingFee = totalWeightLbs < 20 ? 0 : 10;

  // calculate totals dynamically
  const itemsTotal =
    cartItems?.reduce(
      (sum, item) => sum + (item.item.price_cents / 100) * item.quantity,
      0
    ) ?? 0;
