...
Seed the database (automatically deletes existing database):
```bash
PYTHONPATH=. python -m backend.app.seed
```

**Note:** The seed script automatically deletes `backend/sqlite.db` if it exists, ensuring a clean development state.
...
Before committing:
- Remove local artifacts: `.venv`, `__pycache__`, `*.pyc`, `.pytest_cache`
- Note: `backend/sqlite.db` is automatically regenerated by the seed script
